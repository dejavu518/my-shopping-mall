          ...
          <el-table-column :label="$t('txt.TXT_0008')" fixed="right" width="100">
            ...
            <template slot-scope="scope">
              <el-popover :ref="`popover-${scope.$index}`" placement="bottom" style="font-size: 14px;cursor: pointer;font-weight: 400" trigger="click">
                ...
                <!--编辑-->
                <el-button size="mini" @click="handProjectQuotation(scope.$index, scope.row)">{{$t('btn.BTN_159')}}</el-button>
                <!--项目报价-->
                <el-button size="mini" @click="handUploadCDA(scope.$index, scope.row)">{{$t('txt.TXT_1847')}}</el-button>
                <!--签署CDA-->
...
export default {
  ...
  data() {
    return {
      ...
      screenData: ['clientType', 'clientGrade'],
      searchPlaceholder: this.$t('txt.TXT_1464'),
      list: [],
      autoHeight: "",
      params: {}
  ...
  methods: {
    ...
    loadClientList(pageIndex, pageSize, params) {
      ...
      // this.nextTick(() => {
      //   this.$refs.Paging.internalCurrentPage = this.currentPage
      // })
    },
    /**筛选数据**/
    filterData(v) {
